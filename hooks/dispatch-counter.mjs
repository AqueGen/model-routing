...
if (process.argv[2] === "stats" || process.argv[2] === "report") {
  ...
  if (withEffort.length) {
    ...
    // Nothing inherited means nothing for the caveats below to qualify: they
    // explain where an inherited level came from and what it cannot see.
    if (!inherited.length) effortLines.push("", `Effort: all ${withEffort.length} dispatches ran on agent types that pin their own effort.`);
...
if (process.argv[2] === "tokens") {
  ...
  const out = [
    ...
    ...(agentRows.length || metaless ? [
      ...
      // qualifies. The advice names what the amounts compare, a subagent on the
      // session model, and stays pin-neutral: for a role pinned at the tier that
      // ran, going lower is the below-pin case above, not a saving.
      ...(costlier.length ? [`  Routed down but priced higher than staying on the session model: ${costlier.slice(0, 3).map((c) => `${c.agent} on ${shortModel(c.model)} from ${shortModel(c.session)} ${fmtUsd(c.ran)} vs ${fmtUsd(c.inherited)} (cache reads ${cacheReadRate(c.model)} vs ${cacheReadRate(c.session)} per MTok)`).join(", ")}${costlier.length > 3 ? `, and ${costlier.length - 3} more` : ""}. A lower tier is not always a lower rate, and cache reads are usually most of the volume. The cheaper option measured here is dispatching with model=<session model>; a tier lower than the one that ran is an option only where the agent's pin allows it.`] : []),
