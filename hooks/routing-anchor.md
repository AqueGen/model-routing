...
- Implementing an approved plan/spec: `implementer` (sonnet - the value default at a fraction of opus cost); for multi-file/architectural/subtle work dispatch it with `model=opus`, and escalate when in doubt - the opus tier stepped up at unchanged price. Batch related tasks per agent.
