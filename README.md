...
Because that is easy to forget, the plugin checks for a newer version at most once a day on session start and shows a one-line notice when you are behind, carrying your version, the published one, both commands, and a link to the release notes. The check is a single fetch of the published manifest, fails silently offline, and never installs anything - updating stays your call.
